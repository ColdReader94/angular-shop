...
export class FavouriteComponent implements OnInit {
  ...
  constructor(private store: Store<AppState>,
    private favItemsSelector: UserDataSelectors, private httpService: HttpRequestsService) { }

  ngOnInit(): void {
     this.favoritesItems$ = this.store.select(this.favItemsSelector.selectItemsInFavourites).pipe(
      map((itemsIdArray) => {
  ...
  public removeFromList(id: string, event: Event): void {
    ...
    this.store.dispatch(tryToAddToFavourite({
      itemStatusChange: id,
      isFavorite: false,
  }));
  }
